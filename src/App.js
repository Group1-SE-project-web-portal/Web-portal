...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  chartFour = () => {
    ...
    const dashboardFourItems = dashboards[7].dashboardItems;
    console.log(dashboardFourItems)
    const itemsIds = [];
     for (let i = 0; i < dashboardFourItems.length; i++) {
      if (dashboardFourItems[i].chart) {
         itemsIds.push(dashboardFourItems[i].chart);
       }
     }
     let requiredCharts = [];
     charts.forEach(chart => {
    ...
    // prepare chart One
    const chartOneMetadata = requiredCharts[1];
    console.log(chartOneMetadata)
    const chartOneName = chartOneMetadata.displayName
    const periods = 'THIS_YEAR'
    const dataFourDimension = chartOneMetadata.dataDimensionItems[0].programIndicator.id
    console.log(dataFourDimension); 
    const orgUnits = chartOneMetadata.organisationUnits.map(ids => ids.id)

    //get values
     var dataValues = $.ajax({
     url: BASE_URL + `/analytics.json?dimension=dx:${dataFourDimension};&dimension=ou:${orgUnits[0]};${orgUnits[1]};${orgUnits[2]};${orgUnits[3]};${orgUnits[4]}&dimension=pe:${periods}`,
     dataType: "json",
     headers: { "Authorization": "Basic " + btoa(USERNAME + ":" + PASSWORD) },
     success: function (data) { },
     async: false,
...
// /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  render() {
    ...
    return (
      ...
      <div className="App" >
        <NavBar dash1={dashboards[0].displayName} dash2={dashboards[1].displayName} dash3={dashboards[2].displayName} dash4={dashboards[7].displayName}/>
        <Switch>
          ...
          <Route exact path="/dash4" component={() =>
            <Dash1 >
              {this.chartFour()}
            </Dash1>
          } />
